classdef Triggering < scanimage.interfaces.Class
    ...
    methods
        ...
        function delete(obj)
            % disconnect reference clock route
            try
                obj.connectReferenceClock(false);
            catch ME
    ...
    methods (Hidden)    
        ...
        function ziniPrepareTasks(obj)
            ...
            obj.connectReferenceClock(true);
        ...
        function connectReferenceClock(obj,tfConnect)
            deviceName = obj.hLinScan.mdfData.deviceNameAux;
            
            if isempty(obj.hLinScan.trigReferenceClkInInternalTerm)
                termSrc = sprintf('/%s/%s',deviceName,'10MHzRefClock');
                rate = 10e6;
            else
                termSrc = obj.hLinScan.trigReferenceClkInInternalTerm;
                rate = obj.hLinScan.trigReferenceClkInInternalRate;
            end
            
            if obj.hLinScan.mdfData.enableRefClkOutput
                termDest = sprintf('/%s/%s',deviceName,obj.REFERENCE_CLOCK_TERM_OUT);
                if tfConnect
                    obj.hRouteRegistry.connectTerms(termSrc,termDest);
                else
                    obj.hRouteRegistry.disconnectTerms(termSrc,termDest);
                end
            end
            
            %workarround to allow faster timebase for supporting additional
            %sample rates, eg 4MHz for  PXIe-6124
            if ~isempty(obj.hLinScan.mdfData.internalRefClockSrc)
                ...
                obj.referenceClockTermInt = termSrc;
                obj.referenceClockRateInt = rate;
            end
        end
    end
    ...
    methods (Access = private)
        ...
        function [initialDelayTime, highTime, lowTime] = generateFrameClockOutput(obj)
            hRoiGroup = obj.hLinScan.currentRoiGroupScannerCoords;
            ...
            [transitTimes,flybackTime]=arrayfun(@(z) hRoiGroup.transitTimes(scannerset,z),zs,'UniformOutput',false);
