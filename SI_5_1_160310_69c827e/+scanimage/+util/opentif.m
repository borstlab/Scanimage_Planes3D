...
%
    ...
    [frameStringCell] = scanimage.util.private.getStringCellFromTiff(hTif);        

    [verInfo] = scanimage.util.private.getSITiffVersionInfo(frameStringCell{1});        

    bErrorFound = false;
    % NOTE: Reverting to older mechanism. That approach turned out to be too slow for users that have around 
    %       20000 frames per TIFF file. Even at 1000 frames the delay is inconvenient for some users.
    try 
        % Use parsing version prior to this change. 
        % TODO: make sure the following throws an error with older data to trigger try-catch
        [header] = scanimage.util.private.parseHeaderStringCellToStruct_legacy(frameStringCell,verInfo);
    catch
        bErrorFound = true;
    end

    numImages = numel(frameStringCell);
    ...
    if hdr.discardFlybackframesEnabled && strcmp(si_ver,'2015')
