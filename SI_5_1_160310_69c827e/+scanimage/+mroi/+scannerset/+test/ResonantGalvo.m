classdef ResonantGalvo < matlab.unittest.TestCase
    properties
        default
    end
    methods(TestMethodSetup)
        function create(testcase)
            testcase.default=scanimage.mroi.scannerset.ResonantGalvo.default;
        end
    end
    ...
    methods(Test)
        function constructionExample(testcase)
            ...
            testcase.verifyThat(...
                @scanimage.mroi.scannerset.ResonantGalvo.default,...
                IssuesNoWarnings);
        end
        ...
        function scanGenerationExample(testcase)
            ...
            testcase.verifyThat(size(ao,1),IsEqualTo(8081));
            testcase.verifyThat(size(ao,2),IsEqualTo(2));
        end
        
        function differentSizeScanFieldsShouldHaveSameNumberOfLines(testcase)
            import matlab.unittest.constraints.*
            sf1 = scanimage.mroi.scanfield.fields.Rectangle([0.1,0.1,0.1,0.1]);
            ao1 = testcase.default.scan(sf1);            
            sf2 = scanimage.mroi.scanfield.fields.Rectangle([0.1,0.1,0.1,0.2]);
            ao2 = testcase.default.scan(sf2);            
            testcase.verifyThat(ao1,HasSize(size(ao2)));
        end
