classdef ResonantGalvoGalvo < scanimage.mroi.scannerset.ScannerSet
    ...
    properties(Constant)
        CONSTRAINTS = {@scanimage.mroi.constraints.noRotation @scanimage.mroi.constraints.maxWidth @scanimage.mroi.constraints.sameWidth...
                       @scanimage.mroi.constraints.xCenterInRange @scanimage.mroi.constraints.maxHeight @scanimage.mroi.constraints.yCenterInRange...
    ...
    methods(Static)
        function obj=default()
            ...
            obj=scanimage.mroi.scannerset.ResonantGalvoGalvo(r,g,g,[]);
        end
    end
    
    methods
        function obj = ResonantGalvoGalvo(name,resonantx,galvox,galvoy,beams,fillFractionSpatial)
            %% ResonantGalvoGalvo(resonantx,galvox,galvoy,beams)
            %
        ...
        function fovarray = fov(obj)
            ...
            galvoYFov = [-obj.scanners{3}.fullAngleDegrees/2 obj.scanners{3}.fullAngleDegrees/2];
            
            if obj.resonantLimitedFovMode
                fovarray = [resFov galvoYFov];
            else
                galvoXFov = [-obj.scanners{2}.fullAngleDegrees/2 obj.scanners{2}.fullAngleDegrees/2];
                fovarray = [(resFov + galvoXFov) galvoYFov];
            end
        end
        ...
        function ao_volts = pathFovToAo(obj,path_FOV)
            ...
            ao_volts.G(:,1) = obj.fov2volts(path_FOV.G(:,1),2);
            ao_volts.G(:,2) = obj.fov2volts(path_FOV.G(:,2),3);
        ...
        function [path_FOV, seconds] = scanPathFOV(obj,scanfield,roi,actz,dzdt)
            ...
            if obj.resonantLimitedFovMode
                ...
                ctrx = (rect(1)+rect(3)/2) -.5; %center in signed fov coords
                ctrxfov = ctrx * obj.scanners{1}.fullAngleDegrees / obj.scanners{2}.fullAngleDegrees + .5; %convert from resonant fov to galvo fov
                gfov(:,1)= ctrxfov * ones(gsamples,1);
            else
                rscl = (obj.scanners{1}.fullAngleDegrees + obj.scanners{2}.fullAngleDegrees) / obj.scanners{1}.fullAngleDegrees;
                rfov = rect(3) * rscl * ones(rsamples,1);
                
                gscl = (obj.scanners{1}.fullAngleDegrees + obj.scanners{2}.fullAngleDegrees) / obj.scanners{2}.fullAngleDegrees;
                xw = ([rect(1) rect(1)+rect(3)] - .5) * gscl;
                gfov(:,1)=((xw(1) + xw(2))/2 + .5) * ones(gsamples,1);
            end
            gfov(:,2)=linspace(rect(2),rect(2)+rect(4),gsamples);
            ...
            assert(all(path_FOV.G(:,1) <= 1.0001) && all(path_FOV.G(:,1) >= -0.0001), 'Attempted to scan outside X galvo scanner FOV.');
            assert(all(path_FOV.G(:,2) <= 1.0001) && all(path_FOV.G(:,2) >= -0.0001), 'Attempted to scan outside Y galvo scanner FOV.');
        ...
        function position_FOV = mirrorsActiveParkFovPosition(obj)
            ...
            position_FOV(:,3) = obj.degrees2fov(3,obj.scanners{3}.parkAngleDegrees);
        end
        ...
        function path_FOV = zFlybackFrame(obj, frameTime)            
            ...
            path_FOV.G = nan(round(obj.nsamples(2,frameTime)),2);
        ...
        function seconds = transitTime(obj,scanfield_from,scanfield_to)
            ...
            if isnan(scanfield_to)
                seconds = max(obj.scanners{2}.flybackTimeSeconds,obj.scanners{3}.flybackTimeSeconds);
            else
                seconds = max(obj.scanners{2}.flytoTimeSeconds,obj.scanners{3}.flytoTimeSeconds);
            end
        ...
        function v = resonantScanFov(obj, roiGroup)
            ...
            if ~isempty(roiGroup.activeRois) && ~isempty(roiGroup.activeRois(1).scanfields)
                ...
                v = rfov / obj.fillFractionSpatial;
            else
                v = 0;
            end
        end
        ...
        function v = resonantScanVoltage(obj, roiGroup)
            ...
            if ~isempty(roiGroup.activeRois) && ~isempty(roiGroup.activeRois(1).scanfields)
                ...
                v = obj.fov2volts(rfov / obj.fillFractionSpatial,1);
            else
                v = 0;
            end
        end
    ...
    methods(Access=private)
        ...
        function n=numberOfLines(obj,scanfield)
            rect=scanfield.boundingbox();
            n=rect(4)*obj.scanners{3}.pixelCount;
