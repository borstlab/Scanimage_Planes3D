classdef HasMachineDataFile < most.HasClassDataFile
    ...
    properties (Abstract,Constant,Hidden)
        % mdfOptionalVars;
        % Struct. If this property is present in the object, its fieldnames
        % and values will be added to mdfData except an individual fieldname
        % is already present in mdfData
    ...
    methods (Access = protected)
        function obj = HasMachineDataFile(autoLoadMdf, mdfHeading, mdfPath)
            ...
            if autoLoadMdf
                obj.initializeMdf(mdfPath, mdfHeading);
            end
        end
        
        function obj = initializeMdf(obj, mdfPath, mdfHeading)
            ...
            varstruct = obj.getMDFVars(mdfPath, mdfHeading); % can throw
            varstruct = obj.getOptionalVars(varstruct);
    ...
    methods (Hidden)
        ...
        function vars = getMDFVars(obj, mdfPath, mdfHeading)
        ...
        % varstruct
        function varstruct = getOptionalVars(obj,varstruct)
            if ~isprop(obj,'mdfOptionalVars') || isempty(obj.mdfOptionalVars)
                return
            end
            
            assert(isstruct(obj.mdfOptionalVars),'Property mdfOptionalVars needs to be a struct');
            
            if ~isstruct(varstruct) || isempty(varstruct)
                varstruct = struct();
            end
            
            optionalPropNames = setdiff(fieldnames(obj.mdfOptionalVars),fieldnames(varstruct));
            for idx = 1:length(optionalPropNames)
                varstruct.(optionalPropNames{idx}) = obj.mdfOptionalVars.(optionalPropNames{idx});
            end
        end        
    end
        ...
        function tf = ensureClassAndDependentsAreInMDF(mdfCls, mdfHeading)
            ...
            for c = 1:numel(allClasses)
                ...
                if ~mdf.isHeading(hding)
                    ...
                    mdf.copyModelFile(modelfn,hding);
                    tf = true;
                end
            end
        end
...
function fname = zlclGetModelFullFileName(clsName)
    ...
    fname = fullfile(most.util.className(clsName,'classPrivatePath'),[strippedClsName '_model.m']);
end
